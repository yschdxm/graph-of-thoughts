...
from examples.keyword_counting import plot as keyword_counting_plot
...
def get_sorting_prompt_content(length, method_name):
    if length == 32:
...
def run_sorting(input_str: str, method_name: str) -> tuple:
    try:
        ...
        prompt_content = get_sorting_prompt_content(len(nums), method_name)
        ...
        lm = language_models.ChatGPT(config_path, model_name="deepseek", cache=True)
        ...
        state_dict = {"original": str(nums), "current": result}
...
def get_doc_merge_prompt_content(method_name: str, num_documents: int = 4):
    try:
...
def run_doc_merge(method_name: str, sample_id: str, custom_docs: str) -> tuple:
    try:
        ...
        executor = controller.Controller(
            ...
            {"documents": documents, "current": "", "parts": set(), "method": method_name.lower()},
        )
        ...
        cost = lm.cost
        prompt_content = get_doc_merge_prompt_content(method_name, len(documents))
...
def get_keyword_counting_modules():
    return (KeywordCountingPrompter, KeywordCountingParser, 
            {"IO": kw_io, "CoT": kw_cot, "ToT": kw_tot, "ToT2": kw_tot2, "GoT4": kw_got4, "GoT8": kw_got8, "GoTx": kw_gotx})

def get_keyword_counting_prompt_content(method_name):
...
def run_keyword_counting(input_text: str, method_name: str) -> tuple:
    try:
        ...
        method = method_map[method_name]
        prompt_content = get_keyword_counting_prompt_content(method_name)
        ...
        executor = controller.Controller(
            lm, operations_graph, prompter_class(), parser_class(),
            {"original": input_text, "ground_truth": "{}", "current": "", "phase": 0, "method": method_name.lower()},
        )
...
def get_set_intersection_modules(length):
    if length == 32:
        return (SetIntersectionPrompter32, SetIntersectionParser32, 
                {"IO": si_io32, "CoT": si_cot32, "ToT": si_tot32, "ToT2": si_tot232, "GoT": si_got32})
    elif length == 64:
        return (SetIntersectionPrompter64, SetIntersectionParser64, 
                {"IO": si_io64, "CoT": si_cot64, "ToT": si_tot64, "ToT2": si_tot264, "GoT": si_got64})
    elif length == 128:
        return (SetIntersectionPrompter128, SetIntersectionParser128, 
                {"IO": si_io128, "CoT": si_cot128, "ToT": si_tot128, "ToT2": si_tot2128, "GoT": si_got128})
    else:
        raise ValueError(f"不支持的长度 {length}。支持的长度：32, 64, 128")

def get_set_intersection_prompt_content(length, method_name):
    if length == 32:
...
def run_set_intersection(set1_str: str, set2_str: str, method_name: str) -> tuple:
    try:
        ...
        method = method_map[method_name]
        prompt_content = get_set_intersection_prompt_content(len(set1_nums), method_name)
...
with gr.Blocks(title="Graph of Thoughts 多功能演示") as demo:
    ...
    with gr.Tabs():
        ...
        with gr.TabItem("文档合并"):
            ...
            with gr.Row():
                with gr.Column(scale=1):
                    ...
                    gr.Examples(
                        ...
                        inputs=[doc_sample, doc_method, doc_custom],
                        label="点击示例快速填充"
                    )
            ...
            doc_run.click(
                fn=run_doc_merge,
                inputs=[doc_method, doc_sample, doc_custom],
